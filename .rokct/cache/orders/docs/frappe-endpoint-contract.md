...
| `convertDeliveryToCollected` | — (no legacy counterpart) | `seller_order.convert_delivery_to_collected` (`order_id`) | **New, merchants_sdk 1.20.0** (approved strip section 43). ONE atomic call: delivery type → Pickup, driver unassigned, the fee refunded to the customer's wallet or paid to the driver as a callout, order completed. Deliberately NOT a client-orchestrated sequence — the unassign has to land before the order reaches Delivered or the settlement pays the driver the fee twice, and that ordering cannot survive a dropped connection on the client. Idempotent (`already_converted`), which is what makes the till's offline replay safe. |
