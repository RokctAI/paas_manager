    ...
    for ro in repeating_orders:
        ...
        try:
            ...
            if ro.payment_method == "Wallet":
                ...
                try:
                    from {app_name}.wallet.tenant.api.payment.payment import process_token_payment
                    # `ro.saved_card` is the Saved Card docname, and the
                    # docname is the handle the charge takes. The gateway
                    # reuse credential is a Password field on that card: it
                    # stays server-side and is resolved there, so nothing on
                    # this path reads or carries it.
                    ...
                    res = process_token_payment(
                        new_order.name, saved_card=ro.saved_card
                    )
            ...
            if not payment_success:
                ...
                # Notify User
                try:
                    from {app_name}.comms.tenant.api.notification.notification import send_push_notification
