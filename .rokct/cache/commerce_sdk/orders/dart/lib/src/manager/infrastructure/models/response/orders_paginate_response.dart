...
class OrdersPaginateResponse {
  ...
  /// `{data: {statistic, orders}}`, while today's
  /// `seller_order.get_seller_orders` (after base_sdk's `message` unwrap)
  /// returns a bare order list with no statistic block. The list branch keeps
  /// the queues rendering while the statistic gap is closed server-side - see
  /// `docs/frappe-endpoint-contract.md`; counters degrade to zero, nothing is
  /// faked.
  OrdersPaginateResponse.fromJson(dynamic json) {
    if (json is List) {
      _data = OrderResponseData(
        orders: json.map((v) => OrderData.fromJson(v)).toList(),
      );
      return;
    }
    _data =
