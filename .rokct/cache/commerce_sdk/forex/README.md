...
The alternative — depending on `WalletRepositoryFacade` — would put the money-moving sends
(`sendWalletToPhone()`, `sendWalletByCode()`) and `walletTopUp()` within reach of every screen
here, for the sake of reading one number.
