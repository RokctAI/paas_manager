...
def get_shop_coords(shop: Any) -> tuple:
    ...
    either latitude/longitude or lat/long keys, or the GeoJSON
    FeatureCollection / Feature / Point shape that Frappe's Geolocation
    widget writes (``coordinates`` ordered [longitude, latitude]). Accepts
    a shop name, a Shop document, or any dict-like row that carries
    ``location``. Tolerant of missing, empty, or malformed location data.
    """
    import json
    ...
    # GeoJSON, as written by Frappe's Geolocation control: unwrap a
    # FeatureCollection to its first Point feature, a Feature to its
    # geometry, then read Point coordinates ([lng, lat] per the spec).
