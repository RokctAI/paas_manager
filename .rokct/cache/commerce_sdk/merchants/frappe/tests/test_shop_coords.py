...
class TestGetShopCoords(unittest.TestCase):
    ...
    def test_geojson_feature_collection(self):
        # Frappe's Geolocation widget stores a FeatureCollection whose
    ...
    def test_geojson_single_feature(self):
        loc = json.dumps(
            {
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [28.05, -26.1]},
            }
        )
        self.assertEqual(get_shop_coords({"location": loc}), (-26.1, 28.05))

    def test_geojson_bare_point(self):
        loc = json.dumps({"type": "Point", "coordinates": [18.42, -33.92]})
        self.assertEqual(get_shop_coords({"location": loc}), (-33.92, 18.42))

    def test_geojson_empty_feature_collection(self):
        loc = json.dumps({"type": "FeatureCollection", "features": []})
        self.assertEqual(get_shop_coords({"location": loc}), (None, None))

    def test_geojson_non_point_geometry(self):
        loc = json.dumps(
            {
                "type": "FeatureCollection",
                "features": [
    ...
    def test_geojson_short_coordinates(self):
        loc = json.dumps({"type": "Point", "coordinates": [28.05]})
        self.assertEqual(get_shop_coords({"location": loc}), (None, None))
